...
| [aws_iam_policy_document.pull_through_cache_ecr](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) | data source |
...
| <a name="input_pull_time_update"></a> [pull\_time\_update](#input\_pull\_time\_update) | (Optional) A configuration for pull time update. `pull_time_update` as defined below.<br/>    (Optional) `excluded_principals` - One or more IAM principals to exclude from having image pull times recorded. Defaults to `[]`. | <pre>object({<br/>    excluded_principals = optional(set(string), [])<br/>  })</pre> | `{}` | no |
...
| <a name="output_pull_time_update"></a> [pull\_time\_update](#output\_pull\_time\_update) | The configuration for pull time update in ECR registry. |
